...
Authentication & Identity Intelligence (Phase 2E) - Detector (orchestrator)
...
:class:`technology.detector.TechnologyDetector`:
    ...
    extract material (html / inline scripts / script+resource-hint URLs)
        -> match each of the fixed signature tables (providers, identity
           providers, protocols, security features) against every unit,
           plus response header *names* and cookie *names* ->
               score with the shared ConfidenceEngine ->
                   emit Technology objects + an AuthDetectionReport

Design commitments (mirroring :mod:`modules.ai_detection.detector`):

* **One scoring engine.** Every confidence in this package comes from
  :class:`technology.confidence.ConfidenceEngine` via the ``auth_evidence``
  (text-mined) source key, plus the project's existing "headers" and
  "cookies" weights -- reused as-is because a matched header/cookie *name*
  is the same kind of evidence the core engine already scores that way.
...
* **Network-free, always.** Nothing here ever performs I/O.
...
* **Precision over recall.** Every signature pattern is a protocol-, brand-
  or host-specific token (see providers.py/protocols.py/features.py module
  docstrings); nothing here infers a technology from a bare dictionary word.
* **Header/cookie *values* are never inspected**, only the names the server
  chose to send -- so this cannot leak credential-bearing header or cookie
  content, exactly like Phase 2D's header handling.
"""
...
from .features import SECURITY_FEATURES
from .models import (
    AuthCategory,
...
from .providers import IDENTITY_PROVIDERS, PROVIDERS
from .utils import clamp, dedupe

logger = logging.getLogger("webintelpro.auth_detection.detector")

# Source key for text-mined evidence (HTML/JS). Registered in
# ConfidenceEngine.WEIGHTS alongside Phase 2A-2D's "js_bundle"/"js_runtime"/
# "api_evidence"/"ai_evidence". Header-name and cookie-name evidence reuse
# the engine's existing "headers"/"cookies" keys instead of new ones, since
# they are literally the same kind of signal.
AUTH_EVIDENCE_SOURCE = "auth_evidence"

ALL_SIGNATURES: Tuple[Signature, ...] = (
    PROVIDERS + IDENTITY_PROVIDERS + PROTOCOLS + SECURITY_FEATURES
)
...
class _Observation:
    ...
    cookie_evidence: List[str] = field(default_factory=list)
    sources: set = field(default_factory=set)


class AuthDetector:
    """Detects authentication providers, identity providers, protocols and
    security features.

    Passive-only: reads material already present in the fetched HTML/JS plus
    response headers and cookies already fetched for the page. Reuses the
    caller's :class:`ConfidenceEngine` so auth findings are scored on the
    same scale as every other detection source. Construction is cheap and
    network-free.
    """

    def __init__(self, confidence: ConfidenceEngine | None = None,
                 config: AuthDetectionConfig | None = None):
        self.config = config or AuthDetectionConfig()
    ...
    def analyze(self, parsed, base_url: str = "",
               ...
               cookies: Dict[str, str] | None = None) -> AuthDetectionAnalysis:
        """Detect ``parsed``'s authentication stack and return detections +
        report.

        ``parsed`` is a :class:`technology.parser.ParsedHTML`; ``base_url``
        is the page URL (carried for report identity only -- never fetched
        again); ``headers``/``cookies`` are the response headers/cookies
        already fetched for the page. Never raises.
        """
        report = AuthDetectionReport(url=base_url)
        ...
        try:
            ...
            logger.debug("auth detection extraction failed", exc_info=True)
            report.errors.append(f"extraction_failed:{type(exc).__name__}")
            return AuthDetectionAnalysis(technologies=[], report=report)
        ...
        try:
            self._scan_cookies(cookies or {}, observations)
        except Exception:  # noqa: BLE001 - cookie scan must not sink the scan
            logger.debug("cookie scan failed", exc_info=True)
            report.errors.append("cookie_scan_failed")
        ...
        logger.debug("auth detection: %d technology(s), %d finding(s) in %ss",
                     len(tech_by_name), len(report.findings), report.elapsed)
        return AuthDetectionAnalysis(technologies=list(tech_by_name.values()), report=report)
    ...
    def _extract_units(self, parsed) -> Tuple[List[Tuple[str, str]], bool]:
        ...
        :meth:`modules.ai_detection.detector.AIStackDetector._extract_units`.
        """
        config = self.config
    ...
    def _scan_cookies(self, cookies: Dict[str, str], observations: Dict[str, _Observation]) -> None:
        """Match every signature's ``cookies`` against cookie *names*.

        Cookie values are never inspected -- only the names the server chose
        to set -- so this cannot leak session-token content. Unlike header
        field names, cookie names are case-sensitive per RFC 6265, so
        matching (deliberately, unlike :meth:`_scan_headers`) is done without
        ...
        for signature in ALL_SIGNATURES:
            if not signature.cookies:
                continue
            matched_names = [
                name for name in names
                if any(hint in name for hint in signature.cookies)
            ]
            if not matched_names:
                continue
            obs = observations.setdefault(signature.name, _Observation(signature=signature))
            for name in matched_names:
                if name not in obs.cookie_evidence:
                    obs.cookie_evidence.append(name)
            obs.sources.add(DiscoverySource.COOKIE.value)
    ...
    # Emission
    # ------------------------------------------------------------------
    def _emit(self, observations: Dict[str, _Observation], report: AuthDetectionReport,
        ...
        for obs in list(observations.values())[: self.config.max_findings_per_category
            ...
            if obs.text_evidence:
                matches[AUTH_EVIDENCE_SOURCE] = RuleResult(matched=True, evidence=obs.text_evidence)
            if obs.header_evidence:
                matches["headers"] = RuleResult(matched=True, evidence=obs.header_evidence)
            if obs.cookie_evidence:
                matches["cookies"] = RuleResult(matched=True, evidence=obs.cookie_evidence)
            if not matches:
                continue
            ...
            evidence = obs.text_evidence + obs.header_evidence + obs.cookie_evidence
            report.add(AuthFinding(
            ...
            tech_by_name[obs.signature.name] = Technology(
                name=obs.signature.name, category=f"auth_{obs.signature.category.value}",
                confidence=score, version=version,
                evidence=[f"{AUTH_EVIDENCE_SOURCE}:{e}" for e in evidence],
            )
